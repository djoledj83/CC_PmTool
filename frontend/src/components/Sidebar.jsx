...
const NAV_GROUPS = [
    ...
    },
    {
        ...
        // A direct link (no sub-items) — one click opens the admin panel
        // on its first tab; the sections live behind the in-page tab bar.
        to: '/announcements',
        adminOnly: true,
    },
];
...
const NAV_ITEMS = [
    ...
    // ---- Admin -------------------------------------------------------
    // The Admin group itself is a direct link (see NAV_GROUPS `to`), so it
    // needs no sub-items — clicking it opens the tabbed admin panel.
...
function useNavBadges() {
    ...
    return { visibleItems, badgeFor, isAdmin };
}
...
function groupItems(visibleItems) {
    ...
    return NAV_GROUPS
        ...
        // Keep groups that have items, or link-groups (a `to` header that
        // navigates directly and has no sub-items, e.g. Admin).
...
function NavRow({ item, collapsed, onNavigate, badge }) {
    ...
    return (
        <NavLink
            ...
            className={({ isActive }) =>
                cn(
                    // Blended design: the ACTIVE item is a solid purple pill
                    // (concept A); inactive items get a light purple TINT on
                    // hover (concept B). Softer rounded-lg corners throughout.
                    'group relative flex items-center rounded-lg text-sm font-medium transition-all active:scale-[0.97]',
                    collapsed
                        ? 'h-10 w-10 justify-center'
                        : 'gap-3 px-3 py-2',
                    isActive
                        ? 'bg-primary font-semibold text-primary-foreground shadow-[0_6px_16px_-4px_hsl(var(--primary)/0.55)] animate-[sidebar-pill-breathe_3s_ease-in-out_infinite]'
        ...
        >
            {({ isActive }) => (
                <>
                    <Icon className="h-4 w-4 shrink-0 transform-gpu transition-transform duration-200 ease-out group-hover:[transform:perspective(240px)_rotateX(22deg)_translateY(-1.5px)_scale(1.18)]" />
                    {!collapsed && (
                        <span className="truncate">{label}</span>
                    )}
                    {badge != null &&
                        (collapsed ? (
                            <span className="absolute -right-0.5 -top-0.5 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold leading-none text-primary-foreground ring-2 ring-background">
                                {badge > 99 ? '99+' : badge}
                            </span>
                        ) : (
                            <span
                                className={cn(
                                    ...
                                    // On the filled active pill, tint the badge
                                    // with the pill's foreground so it stays legible.
                                    isActive
...
function NavList({ collapsed = false, onNavigate }) {
    const { visibleItems, badgeFor, isAdmin } = useNavBadges();
    ...
    return (
        ...
        >
            {groups.map((group, idx) => {
                // Link-group (has `to`, no sub-items): render as a single
                // direct nav row — one click navigates, no expand step.
                if (group.to) {
                    return (
                        <div
                            key={group.id}
                            className="flex animate-[sidebar-item-in_0.4s_ease-out_both] flex-col"
                            style={{
                ...
                if (collapsed) {
                    return (
                        <div
                            key={group.id}
                            className="flex animate-[sidebar-item-in_0.4s_ease-out_both] flex-col items-center"
                ...
                return (
                    ...
                    >
                        ...
                        >
                            <span
                                className={cn(
                                    'flex h-6 w-6 shrink-0 transform-gpu items-center justify-center rounded-md border border-border/70 bg-background shadow-sm transition-transform duration-200 ease-out group-hover/header:[transform:perspective(240px)_rotateX(18deg)_translateY(-1px)_scale(1.08)]',
                                    group.accent,
                                )}
                            >
                                <GroupIcon className="h-3.5 w-3.5" />
                            </span>
                            <span className="flex-1 text-left text-[11px] font-semibold uppercase leading-none tracking-wider text-muted-foreground">
                                {group.label}
                            </span>
                            {!open && (
                                <span
...
function SidebarUserHeader({ collapsed, onNavigate }) {
    ...
    if (collapsed) {
        return (
            <div className="mt-auto flex flex-col items-center gap-1.5 border-t p-2">
                <button
                    type="button"
                    onClick={viewProfile}
    ...
    return (
        <div className="mt-auto border-t bg-card/40 p-2">
...
export function Sidebar() {
    ...
    return (
        <>
            ...
            <aside
                className={cn(
                    'hidden h-screen shrink-0 flex-col border-r bg-muted/30 bg-gradient-to-b from-primary/[0.06] via-transparent to-transparent transition-[width] duration-150 md:flex',
                    collapsed ? 'w-14' : 'w-60',
                )}
            >
                <div
            ...
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
                <SheetContent
                    side="left"
                    className="flex w-64 max-w-[85%] flex-col bg-gradient-to-b from-primary/[0.06] via-transparent to-transparent p-0"
                >
